...
type Phase = "loading" | "name-prompt" | "matching" | "countdown" | "game" | "round-result" | "result" | "error";

export default function RandomMatchRoomPage() {
  ...
  const socketRef = useRef(getRandomSocket());
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const info = await getRandomRoomInfo(code);
      if (cancelled) return;

      if (!info) {
        setError("Match not found. Try finding another opponent.");
        setPhaseTracked("error");
        return;
      }

      if (info.status === "expired") {
        setError("This match has expired. Try another one!");
        setPhaseTracked("error");
        return;
      }

      if (info.status === "completed") {
        setError("This match is already finished.");
        setPhaseTracked("error");
        return;
      }
      ...
      setIsBotMatch(!!info.isBotMatch);

      const saved = getSavedPlayerName();
      if (saved) {
        setPlayerName(saved);
        setNameInput(saved);
        setPhaseTracked("matching");
        connectSocket(saved);
      } else {
  ...
  const handleSeriesAccepted = useCallback(() => {
    /* server emits series-accepted; handled in listener above */
  }, []);
    ...
    return (
      <main className="ch-room">
        ...
        <div className="ch-room__error">
          ...
          <h2>Oops!</h2>
          <p>{error}</p>
          <button type="button" className="ch-room__error-btn" onClick={() => router.push("/challenge/random")}>
            Find Another Match
          </button>
        </div>
        {modals}
    ...
    return (
      <main className="ch-room">
        ...
        <div className="ch-room__name-prompt">
          ...
          <h2 className="ch-room__name-title" style={{ marginTop: 12 }}>Connecting…</h2>
          <p className="ch-room__name-subtitle">
            {opponentName ? `Get ready — ${opponentName} is here!` : "Waiting for your opponent to join…"}
          </p>
        </div>
        {modals}
      </main>
